...
export function calculatePoolReward(height: number): BigNumber {
  ...
  if (height < 6 * BLOCKS_PER_YEAR) {
    return MOJO_PER_CHIA.times('100').times(POOL_REWARD);
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return MOJO_PER_CHIA.times('50').times(POOL_REWARD);
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return MOJO_PER_CHIA.times('25').times(POOL_REWARD);
  }

  return MOJO_PER_CHIA.times('12.5').times(POOL_REWARD);
}
...
export function calculateBaseFarmerReward(height: number): BigNumber {
  ...
  if (height < 6 * BLOCKS_PER_YEAR) {
    return MOJO_PER_CHIA.times('100').times(FARMER_REWARD);
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return MOJO_PER_CHIA.times('50').times(FARMER_REWARD);
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return MOJO_PER_CHIA.times('25').times(FARMER_REWARD);
  }

  return MOJO_PER_CHIA.times('12.5').times(FARMER_REWARD);
}
