...
import { Offers } from '@poligon/icons';
import {
  ...
  const resolvedOfferInfo = Object.entries(summary[summaryKey]).map(
    ([assetId, amount]) => {
      ...
      if (assetType === OfferAsset.NFT) {
        ...
        displayName = assetIdInfo?.displayName ?? t`Unknown token`;
      }
      return {
        displayAmount,
        displayName,
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'TPOL';
        ...
        <Grid container>
          <Grid xs={12} md={6} lg={5} item>
            <CardHero>
              ...
              <Typography variant="body1">
                <Trans>
                  Create an offer to exchange assets including POL, tokens, and
