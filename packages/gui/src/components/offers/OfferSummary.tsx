...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['pol', 'tpol'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToChia(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['pol', 'tpol'].includes(takerEntries[0][0].toLowerCase())
                ...
                <Flex flexDirection="row" gap={1}>
                  {tradeSide === 'sell' && (
                    <StyledWarningText variant="caption">
                      <Trans>
                        Warning: Verify that the offered tokens asset IDs match the
